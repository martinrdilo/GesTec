<?php
  ...
  $sqlTabla1 = "SELECT O.*, U.apellido, U.nombre, P.tipo, P.marca, P.modelo, OT.nombreTipoOrd, OE.nombreEstadoOrd
  ...
  $sqlTabla2 = "SELECT O.*, U.apellido, U.nombre, P.tipo, P.marca, P.modelo, OT.nombreTipoOrd, OE.nombreEstadoOrd
          FROM ordenes O JOIN usuarios U ON O.idCliente = U.idUsuario
                         JOIN ordenes_tipos OT ON O.idTipo = OT.idTipo
                         JOIN ordenes_estados OE ON O.idEstado = OE.idEstado
                         JOIN productos P ON P.idProducto = O.idProducto
  ...
  $sqlTabla3 = "SELECT O.*, U.apellido, U.nombre, P.tipo, P.marca, P.modelo, OT.nombreTipoOrd, OE.nombreEstadoOrd
              FROM ordenes O JOIN usuarios U ON O.idCliente = U.idUsuario
                         JOIN ordenes_tipos OT ON O.idTipo = OT.idTipo
                         JOIN ordenes_estados OE ON O.idEstado = OE.idEstado
                         JOIN productos P ON P.idProducto = O.idProducto
          ...
          <section class="col s11 m11 l11">
            <div class="row">
              <ul class="collapsible col offset-s1 s11 m12 l12 teal lighten-5" data-collapsible="accordion">
                <li>
                  <div class="collapsible-header active grey darken-1 white-text opcion2"><span class="segundo"><i class="material-icons right">arrow_drop_up</i></span><span class="segundo"><i class="material-icons right" style="display: none">arrow_drop_down</i></span><span class="truncate">Presupuestos en espera de aprobación </span></div>
                  <div class="collapsible-body teal lighten-5">
                    <table class="responsive-table striped highlight display" id="table_dos">
                ...
                <li>
                  ...
                  <div class="collapsible-body teal lighten-5">
                    ...
                    <table class="responsive-table striped highlight display nowrap" id="table_tres">
                      ...
                      <tbody>
                        <?php while ($orden = mysqli_fetch_array($rsTabla3)) : ?>
                          <tr>
                            <td><?= $orden["idOrden"] ?></td>
                            <td><?= $orden["fechaIngreso"] ?></td>
                            <td><?= $orden["apellido"] . " " . $orden["nombre"] ?></td>
